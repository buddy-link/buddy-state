...
var useBuddyState = (key, selector) => {
  ...
  const [value, setValue] = import_react.useState(source?.getValue());
  ...
  const selectedValue = import_react.useMemo(() => selector ? selector(value) : value, [value, selector]);
...
var createEventBus = (initialState) => {
  ...
  return {
    getSource(id) {
      ...
      if (!next_source)
        console.log(`Could not find buddyState for ${id}, please add to initialState`);
      return next_source;
    },
    update(id, value) {
...
var initBuddyState2 = (initialState) => {
